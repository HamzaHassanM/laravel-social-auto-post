...
### Added
- **Laravel Events**: Added `SocialPostPublishing`, `SocialPostPublished`, and `SocialPostFailed` events to the posting lifecycle to allow applications to hook into the process (ideal for SaaS and database state syncing).
- **Documentation Restructure**: Moved detailed setup and advanced usage guides into a dedicated `docs/` folder to keep the `README.md` clean and scannable.
...
### Changed
- **Framework Compatibility**: Expanded Laravel framework compatibility from `^13.0` to `^11.0|^12.0|^13.0` to support a wider range of legacy and current applications.

## [2.3.3] - 2026-05-23

### Fixed
- **Packagist Sync**: Removed hardcoded `version` string from `composer.json` which was causing tag validation mismatches on Packagist and preventing new versions from being deployed.

## [2.3.2] - 2026-05-23

### Added
- **Community Health**: Added `LICENSE` (MIT), `CODE_OF_CONDUCT.md`, `SECURITY.md`, and GitHub Issue/PR templates to standardize open-source contributions.
...
### Updated
- **Laravel 13 Support**: Updated Laravel framework compatibility to ^13.0
- **PHPUnit 12**: Updated test dependencies to ^12.0
- **Orchestra Testbench 11**: Updated testing dependencies to ^11.0
